...
export default function Settings() {
  ...
  const [mainTab, setMainTab] = useState<SettingsTab>(
  ...
  const canAddProfile =
    ...
    !config.profiles.some((p) => p.name === trimmedProfileName);
  ...
  const profilesPanel = activeProfile ? (
    <Box>
      <SectionHeader
      ...
      >
        ...
        <Button
          ...
          onClick={async () => {
            await addProfile(trimmedProfileName);
            setNewProfileName("");
          }}
          sx={{
            height: 36,
  ...
  return (
    ...
    >
      <Tabs
        ...
        value={mainTab}
        onChange={(_e, value: SettingsTab) => setMainTab(value)}
        sx={{
          borderRight: 1,
          borderColor: "divider",
          minWidth: 180,
      ...
      >
        {mainTab === SettingsTab.Appearance && appearancePanel}
        {mainTab === SettingsTab.Profiles && profilesPanel}
        {mainTab === SettingsTab.Integration && integrationPanel}
        {mainTab === SettingsTab.Update && updatePanel}
      </Box>
    </Box>
  );
}
